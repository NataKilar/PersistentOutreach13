/datum/computer_file/program/email_administration
	...
	extended_desc = "This program may be used to administrate EXONET's emailing service."
...
/datum/nano_module/program/email_administration/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = GLOB.default_state)
	...
	if(error)
		...
		for(var/datum/computer_file/data/email_account/account in exonet.email_accounts)
...
/datum/nano_module/program/email_administration/Topic(href, href_list)
	...
	if(href_list["ban"])
		...
		exonet.add_log_with_ids_check("EMAIL LOG: SA-EDIT Account [current_account.login] has been [current_account.suspended ? "" : "un" ]suspended by SA [I.registered_name] ([I.assignment]).")
	...
	if(href_list["changepass"])
		...
		exonet.add_log_with_ids_check("EMAIL LOG: SA-EDIT Password for account [current_account.login] has been changed by SA [I.registered_name] ([I.assignment]).")
		return 1
	...
	if(href_list["viewaccount"])
		for(var/datum/computer_file/data/email_account/email_account in exonet.email_accounts)
