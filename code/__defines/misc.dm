...
#define BOMBCAP_FLASH_RADIUS (GLOB.max_explosion_range*1.5)
									// network module-configuration values. Do not change these. If you need to add another use larger number (5..6..7 etc)
#define NETWORK_SOFTWAREDOWNLOAD 1 	// Downloads of software from network
...
// Network transfer speeds, used when downloading/uploading a file/program.
...
#define NETWORKSPEED_DOS_AMPLIFICATION 5	// Multiplier for Denial of Service program. Resulting load on network relay is this multiplied by NETWORKSPEED of the device
